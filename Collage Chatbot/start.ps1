...
$distIndex = "$ScriptDir\frontend\dist\index.html"
...
python -c "import sys; sys.path.insert(0, r'$ScriptDir'); from backend.app.database import Base, engine; from database.seed.seed_data import seed_database; Base.metadata.create_all(bind=engine); seed_database()"
...
Write-Host "[6/6] Starting Unified Server on 5001 ... OK" -ForegroundColor Green
...
Write-Host "--------------------------------------------------------------------------------" -ForegroundColor Green
Write-Host "  * Unified Port    : 5001 (No separate backend/frontend ports required)" -ForegroundColor White
...
Write-Host "  * Metrics (Prom)  : http://localhost:5001/metrics" -ForegroundColor White
